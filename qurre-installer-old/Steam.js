...
module.exports = class Steam {
    constructor(options, downloadCB = ()=>{}) {
    ...
    async initialize(downloadCB = ()=>{}){
        ...
        var args = ['@ShutdownOnFailedCommand 1', '@NoPromptForPassword 1', 'force_install_dir ' + this.installDir, 'login anonymous', 'app_update 996560'].concat('quit').map(function (x) {
          return '+' + x
        }).join(' ').split(' ')
        ...
        child.stdout.on('data', (data)=>console.log(data.toString('ascii')))
